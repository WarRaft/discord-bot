...
impl JobTailcut {
    pub(crate) const COLLECTION: &'static str = "discord_command_tailcut";
    pub(crate) const MAX_RETRIES: u32 = 3;

    /// Count total number of tailcut tasks
    pub async fn count_total(db: &mongodb::Database) -> Result<u64, BotError> {
        let collection: Collection<JobTailcut> = db.collection(Self::COLLECTION);
