...
impl Command for Png {
    ...
    async fn handle(interaction: Interaction) -> Result<()> {
        ...
        // Get queue statistics for PNG conversion
        let queue_info = match crate::db::blp_queue::BlpQueueItem::count_total_by_type(
            ...
            Ok(count) => {
                if count > 0 {
                    format!("📊 **Usage Statistics:** {} total PNG conversions processed", count)
                } else {
                    "📊 **Usage Statistics:** No PNG conversions yet".to_string()
                }
            }
            Err(_) => "📊 **Usage Statistics:** Unable to check statistics".to_string()
        };

        let info_text = format!("🖼️ **PNG Image Conversion**\n\n\
**Usage:**\n\
• Mention the bot with BLP attachments: `@Raft png [zip]`\n\
• Add `zip` to receive files in a ZIP archive\n\n\
**Examples:**\n\
• `@Raft png` - Convert BLP files to PNG\n\
• `@Raft png zip` - Convert BLP files to PNG and ZIP the results\n\n\
**File size limit:** 25MB per file\n\
**Multiple files:** Yes, attach multiple BLP images in one message\n\
**Quality:** PNG conversion preserves original quality\n\n\
**Note:** This converts BLP texture files used in Warcraft III back to standard PNG images.\n\n\
{}\n\n\
